...
export default function MBlogSection() {
    ...
    <section className="max-w-6xl mx-auto px-4 py-8 relative group">
      ...
      <div className="relative">
            ...
            <SwiperSlide key={blog.id} className="h-auto flex">
              <div className="bg-white rounded-2xl shadow-md hover:shadow-xl transition-shadow duration-300 flex flex-col justify-between overflow-hidden border border-gray-100 w-full h-full">
                ...
                <div className="relative w-full h-56 bg-gray-100 overflow-hidden rounded-t-2xl">
                  ...
                  {/* Tag Badge Overlay */}
                  {blog.tags.length > 0 && (
                    <div className="absolute top-4 left-4 bg-red-600 text-white text-xs font-semibold px-3 py-1 rounded-md shadow-sm z-10">
                      {blog.tags[0]}
                    </div>
                  )}
                </div>
                ...
                <div className="relative px-6 pt-4 flex-1 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center space-x-2 mb-3">
                      ...
                      <span className="w-6 h-2 bg-red-600 rounded-full inline-block transform -rotate-45"></span>
                    ...
                    <h2 className="text-lg font-bold text-gray-800 leading-snug hover:text-slate-500 transition-colors line-clamp-2">
                      {blog.title}
                    </h2>
                  ...
                  <div className="pt-6 pb-6">
                    <Link
                      href={blog.link}
                      className="w-full flex items-center justify-center space-x-2 bg-red-600 hover:bg-red-800 text-white font-semibold py-2.5 px-4 rounded-xl transition-colors duration-200 shadow-sm"
                    >
                      <span>Read More</span>
                      <ArrowRight size={16} />
                    </Link>
        ...
        <button
          ...
          className="custom-prev absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 z-10 bg-slate-800 text-white p-3 rounded-full shadow-lg hover:bg-red-600 transition-colors duration-300 focus:outline-none"
        >
          <ChevronLeft size={24} />
        </button>
        ...
        <button
          ...
          className="custom-next absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 z-10 bg-slate-800 text-white p-3 rounded-full shadow-lg hover:bg-red-600 transition-colors duration-300 focus:outline-none"
        >
