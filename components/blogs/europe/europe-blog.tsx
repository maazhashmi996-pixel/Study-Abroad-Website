...
export default function EuropeBlogSection() {
  return (
    <section className="bg-slate-50 py-12 px-4 sm:px-6 lg:px-8 border-t border-slate-200">
      <div className="max-w-6xl mx-auto relative group">
        ...
        <div className="text-center mb-10">
          <span className="inline-block bg-[#E29D12] text-white text-xs font-extrabold px-3 py-1 rounded-full uppercase tracking-wider mb-3 shadow-sm">
            Latest Insights
          </span>
          <h2 className="text-3xl md:text-4xl font-extrabold text-[#462066] tracking-tight">
            Study Abroad Blogs <span className="text-[#E29D12]">— Guidance, Tips &amp; More</span>
          </h2>
          <p className="mt-2 text-slate-600 text-sm md:text-base max-w-xl mx-auto">
            Get expert insights, step-by-step visa processes, and admission guidance for top global destinations.
          </p>
        </div>
            ...
            <SwiperSlide key={blog.id} className="h-auto">
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between overflow-hidden h-full group/card">
                ...
                <div className="relative w-full h-52 bg-slate-100 overflow-hidden rounded-t-2xl">
                  ...
                  {/* Tag Badge Overlay */}
                  {blog.tags.length > 0 && (
                    <div className="absolute top-4 left-4 z-10 bg-[#E29D12] text-white text-xs font-bold px-3 py-1 rounded-full shadow-md uppercase tracking-wider">
                      {blog.tags[0]}
                    </div>
                  )}
                </div>
                ...
                <div className="p-6 flex-1 flex flex-col justify-between">
                  <div>
                    <h3 className="text-lg font-bold text-slate-900 leading-snug mb-2 group-hover/card:text-[#462066] transition-colors line-clamp-2">
                      {blog.title}
                    </h3>
                  ...
                  <Link
                    href={blog.link}
                    className="inline-flex items-center justify-center gap-2 w-full bg-[#462066] hover:bg-[#E29D12] text-white font-semibold py-2.5 px-4 rounded-xl transition-colors duration-200 text-sm shadow-sm"
                  >
        ...
        <button 
          ...
          className="blog-slider-prev absolute left-0 top-1/2 -translate-y-1/2 -translate-x-3 md:-translate-x-5 z-10 bg-[#462066] text-white p-3 rounded-full shadow-lg hover:bg-[#E29D12] transition-colors duration-300 focus:outline-none"
        >
          <ChevronLeft size={20} />
        </button>
        ...
        <button 
          ...
          className="blog-slider-next absolute right-0 top-1/2 -translate-y-1/2 translate-x-3 md:translate-x-5 z-10 bg-[#462066] text-white p-3 rounded-full shadow-lg hover:bg-[#E29D12] transition-colors duration-300 focus:outline-none"
        >
